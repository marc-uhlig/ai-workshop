...
- **THEN** they SHALL verify that each theme exposes exactly four occasions, that Cyberpunk exposes "Signal Boost", "Mission erfüllt", "Starker Move", and "Upgrade verdient", and that ASCII-Art exposes "Danke dir", "Großartige Arbeit", "Stark im Team", and "Erfolg verdient"
...
#### Scenario: ASCII-Art end-to-end creation
- **WHEN** the end-to-end test selects ASCII-Art and one of its occasions before completing the editor
- **THEN** it SHALL verify a native text preview with the selected occasion and a PNG download event
